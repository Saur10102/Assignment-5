    ...
    try {
        let transporter = nodemailer.createTransport({
            ...
            auth: {
                ...
                user: "saurabhkute321@gmail.com",
                pass: "gjmgbxtedktfkqvs"
            }
        })
        ...
        transporter.sendMail(mailOptions, function (err, info) {
            ...
            else console.log("Email sent Sucessfully", info.response)
        })
...
const verifyMail = async (req, res) => {
    try {
        ...
        if (verified) {
            ...
            console.log("Please verify the mail!")
        }
...
let userRegister = async (req, res, filename) => {
    ...
    try {
        ...
        if (userData) {
            ...
            res.render('register', { succMsg: " Verification link is sent to your registered email Id!" })
        }
        else {
        ...
        if (userData) {
            ...
            if (passwordCheck) {
                ...
                if (userData.status == 0) {
                    res.render('login', { errMsg: "Account verification is pending" })
                } else {
                    session = req.session;
                    session.email = email;
        ...
        if (user) {
            ...
            transporter.sendMail(mailOptions, function (err, info) {
                if (err) console.log(err);
                    ...
                    res.render('forget', { succMsg: "Reset password link is send to your registered mail" })
                }
            })
        }
        else {
